    ...
    fn new() -> Self {
        ...
        let _requests_served = meter
            ...
            .with_callback(|observer: &dyn AsyncInstrument<u64>| {
                ...
                observer.observe(monitoring.get_requests_served() as u64, &[]);
            })
            .build();
        ...
        let _requests_per_sec = meter
            ...
            .with_callback(|observer: &dyn AsyncInstrument<f64>| {
                let monitoring = get_monitoring_state();
                observer.observe(monitoring.get_requests_per_sec(), &[]);
            })
            .build();
        ...
        let _active_connections = meter
            ...
            .with_callback(|observer: &dyn AsyncInstrument<u64>| {
                let monitoring = get_monitoring_state();
                observer.observe(monitoring.get_active_connections() as u64, &[]);
            })
            .build();
        ...
        let _uptime_seconds = meter
            ...
            .with_callback(|observer: &dyn AsyncInstrument<u64>| {
                let monitoring = get_monitoring_state();
                observer.observe(monitoring.get_uptime_seconds(), &[]);
            })
            .build();
        ...
        let _file_cache_enabled = meter
            ...
            .with_callback(|observer: &dyn AsyncInstrument<u64>| {
                let monitoring = get_monitoring_state();
                observer.observe(if monitoring.get_file_cache_enabled() { 1 } else { 0 }, &[]);
            })
            .build();
        ...
        let _file_cache_current_items = meter
            ...
            .with_callback(|observer: &dyn AsyncInstrument<u64>| {
                let monitoring = get_monitoring_state();
                observer.observe(monitoring.get_file_cache_current_items() as u64, &[]);
            })
            .build();
        ...
        let _file_cache_max_items = meter
            ...
            .with_callback(|observer: &dyn AsyncInstrument<u64>| {
                let monitoring = get_monitoring_state();
                observer.observe(monitoring.get_file_cache_max_items() as u64, &[]);
            })
            .build();

        let _file_not_found_cache_current_items = meter
            .u64_observable_gauge("gruxi_file_not_found_cache_current_items")
            .with_description("Current number of items in the file not found cache")
            .with_callback(|observer: &dyn AsyncInstrument<u64>| {
                let monitoring = get_monitoring_state();
                observer.observe(monitoring.get_file_not_found_cache_current_items() as u64, &[]);
            })
            .build();

        let _file_not_found_cache_max_items = meter
            .u64_observable_gauge("gruxi_file_not_found_cache_max_items")
            .with_description("Maximum number of items the file not found cache can hold")
            .with_callback(|observer: &dyn AsyncInstrument<u64>| {
                let monitoring = get_monitoring_state();
                observer.observe(monitoring.get_file_not_found_cache_max_items() as u64, &[]);
            })
            .build();

        MetricsExporter { registry, _provider: provider }
    }
}

static METRICS_EXPORTER: OnceLock<MetricsExporter> = OnceLock::new();

pub fn render_metrics() -> String {
    let exporter = METRICS_EXPORTER.get_or_init(MetricsExporter::new);
