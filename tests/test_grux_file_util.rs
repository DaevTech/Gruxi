...
fn test_relative_path_is_expanded() {
    ...
    let result = get_full_file_path(rel).unwrap();
...
fn test_absolute_windows_path_cross_platform() {
    ...
    let result = get_full_file_path(win_abs).unwrap();
    assert_eq!(result, expected);
}
...
fn test_absolute_linux_path() {
    ...
    let result = get_full_file_path(abs).unwrap();
    assert_eq!(result, expected);
}

#[test]
fn test_split_path_unix_path() {
    let (dir, file) = split_path("/path1/path2/index.php");
...
fn test_split_path_windows_path() {
    let (dir, file) = split_path("C:\\path1\\path2\\index.php");
...
fn test_split_path_root_file() {
    let (dir, file) = split_path("C:\\file.txt");
    assert_eq!(dir, "C:/");
